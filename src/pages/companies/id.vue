  ...
  <Form ref="form" :model="currentData" :rules="rules" :label-width="60">
    ...
    <Row>
      ...
      <Col :xs="24" :sm="16">
        <FormItem label="地址" prop="address">
          <Input v-model="currentData.address" size="small" :disabled="disabled"></Input>
        </FormItem>
      </Col>
    </Row>
    ...
    <Collapse>
      ...
      <Panel name="detail">
        ...
        <Row slot="content">
          <Col :xs="24">
            <FormItem label="描述">
              <Input type="textarea" :autosize="{minRows: 2}" v-model="currentData.description" size="small" :disabled="disabled"></Input>
            </FormItem>
          </Col>
          <Col :xs="18" :sm="6">
            <FormItem label="邮编">
              <Input v-model="currentData.zipcode" size="small" :disabled="disabled"></Input>
            </FormItem>
          </Col>
          <Col :xs="24" :sm="16">
            <FormItem label="网址">
              <Input v-model="currentData.site" size="small" :disabled="disabled"></Input>
            </FormItem>
          </Col>
          <Col :xs="16" :sm="6">
            <FormItem label="级别">
              <InputNumber v-model="currentData.level" size="small" :disabled="disabled"></InputNumber>
            </FormItem>
          </Col>
          <Col :xs="16" :sm="8">
            <FormItem label="资本">
              <Input v-model="currentData.fund" size="small" :disabled="disabled"><template slot="append">万元</template></Input>
            </FormItem>
          </Col>
          <Col :xs="16" :sm="8">
            <FormItem label="规模">
              <Input v-model="currentData.staff" size="small" :disabled="disabled"><template slot="append">人</template></Input>
            </FormItem>
          </Col>
          <Col :xs="24">
            <FormItem label="备注">
      ...
      <Panel name="billing">
        ...
        <div slot="content">
          <FormItem slot="content" label="税号">
            <Input v-model="currentData.billing.taxNum" placeholder="请输入企业税号" size="small" :disabled="disabled"></Input>
          </FormItem>
          <FormItem label="银行">
            <Input v-model="currentData.billing.bank" placeholder="请输入开户银行" size="small" :disabled="disabled"></Input>
          </FormItem>
          <FormItem label="账号">
            <Input v-model="currentData.billing.account" placeholder="请输入银行账号" size="small" :disabled="disabled"></Input>
          </FormItem>
          <FormItem label="抬头">
            <Input v-model="currentData.billing.title" placeholder="请输入账户抬头" size="small" :disabled="disabled"></Input>
          </FormItem>
        </div>
      </Panel>
      <!-- billing pane end -->
    ...
    <Row v-if="currentUser && (currentUser.role < 1 || currentUser._id === currentData.createdBy)">
      <Col :xs="24" :sm="12">
        <FormItem label="只读共享" :label-width="74">
          <Select v-model="currentData.readers" multiple filterable size="small" :disabled="disabled" placeholder="可多选">
            <Option v-for="item in users" :label="item.name" :value="item._id" :key="item._id" :disabled="currentData.editors.includes(item._id)">{{ item.name }} ( {{ item.username }} )</Option>
          </Select>
        </FormItem>
      </Col>
      <Col :xs="24" :sm="12">
        <FormItem label="编辑共享" :label-width="74">
          <Select v-model="currentData.editors" multiple filterable size="small" :disabled="disabled" placeholder="可多选">
            <Option v-for="item in users" :label="item.name" :value="item._id" :key="item._id" :disabled="currentData.readers.includes(item._id)">{{ item.name }} ( {{ item.username }} )</Option>
          </Select>
        </FormItem>
      </Col>
    </Row>
    ...
    <div v-if="!embed">
      <FormItem label="联系信息" :label-width="75">
        <Button :disabled="disabled || !currentData._id" size="small" @click="addContact" icon="plus"></Button>
        <span v-if="disabled || !currentData._id">（请先提交新建公司数据）</span>
      </FormItem>
      <Collapse>
        <Panel v-for="(contact, i) in contacts" :ref="'contact' + i" :name="contact.name" :key="contact._id">
          ...
          <Poptip v-if="contact._id" confirm placement="right" transfer :title="'操作不可恢复，是否确定删除该联系人 ' + contact.name + '？'" @on-ok="delContact(i)">
            <Button icon="trash-b" size="small" :disabled="disabled"></Button>
          </Poptip>
          <Button v-else icon="trash-b" size="small" @click.stop.prevent="delContact(i)"></Button>
          <Contact slot="content" :data="contact" :embed="true" :readonly="disabled"></Contact>
        </Panel>
      </Collapse>
    </div>
  </Form>
...
export default {
  ...
  methods: {
    ...
    async setData (id) {
      try {
        let data = await this.$store.dispatch(this.GET_ACTION, this.embed ? id : { id, withContacts: true })
        if (data) {
    ...
    async fetchContacts (companyId) {
      let ret = await this.$store.dispatch('FETCH_CONTACTS', { companyId })
