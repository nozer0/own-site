<template>
  <div>
    <Form inline :label-width="38">
      ...
      <FormItem v-show="!('mine' in $route.query)" label="创建人" :label-width="51">
        <Select filterable clearable size="small" @on-change="handleCreatorQuery">
          <Option v-for="item in users" :label="item.name" :value="item.id" :key="item.id">{{ item.name }} ( {{ item.username }} )</Option>
        </Select>
      </FormItem>
    ...
    <Modal title="客勤记录" v-model="isDialogVisible" width="720" transfer>
      <Visit ref="visit" :embed="true" :readonly="readonly" :data="currentData" @save="handleSave" @cancel="handleCancel"></Visit>
      <div slot="footer"></div>
    </Modal>
  </div>
</template>
...
export default {
    ...
    columns () {
      ...
      return [
        ...
        },
        {
          ...
          width: 170,
          render (h, { row }) {
            return toDateString(row.created)
          }
        }
      ]
    },
  ...
  methods: {
    ...
    getQuery (query) {
      ...
      return user.role > 0 ? { createdBy: user.id } : {}
    }
  },
